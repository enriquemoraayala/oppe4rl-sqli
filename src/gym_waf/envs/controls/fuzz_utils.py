...
def random_string(max_len=7, spaces=True):
    ...
    if max_len == 7:
        min = 5
    return "".join(
        [random_char(spaces=spaces) for i in range(random.randint(min, max_len))]
    )
